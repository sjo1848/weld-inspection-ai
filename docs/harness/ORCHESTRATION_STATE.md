...
| B4 Model promotion | **B4.1 PASS / B4.2 HELD-OUT PASS / B4.3 ACTIVE** | Export, parity-check and freeze task model contract | frozen manifest/test evidence complete; ONNX identity + parity/runtime evidence next |
...
- Final promoted ONNX artifact does not exist yet; B4.3 is active.
...
B4.3 is authorized inside the active Build contract. Export the exact B3 checkpoint to FP32 ONNX using the raw-output contract (`decode_in_inference=False`), record artifact identity/size/opset/tensor contract, prove PyTorch → Python ONNX Runtime parity on fixed non-test inputs, then prove the same artifact executes through ONNX Runtime Web/WASM with bounded numeric variance.

Do not reopen model tuning or rerun the consumed frozen test.
